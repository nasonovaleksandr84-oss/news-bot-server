...
1. Сделай НЕ МЕНЬШЕ 3 разных поисковых запросов через web search по теме геймификации, лояльности, поведенческой экономики, мотивации в бизнесе. Меняй формулировки: разные индустрии (ритейл/банки/HR/EdTech), разные регионы, разные механики. Геймификация — широкая тема, материал всегда есть.
2. ОБЯЗАТЕЛЬНО добавляй в запрос текущий год (${new Date().getFullYear()}) для свежести.
3. Из найденного выбери лучший материал, который заполняет пробел по истории (тип/регион/механика/индустрия).
4. Напиши пост строго по структуре выбранного типа.
5. В поле sourceCitation поставь дословную цитату из статьи — доказательство что источник реальный.
6. Пиши только на основе реального текста статьи, не выдумывай цифры.

ПУСТОЙ ОТВЕТ [] ДОПУСТИМ ТОЛЬКО как крайний случай — если после 3+ разных запросов реально ничего нет по теме. Это почти никогда не должно случаться. Не сдавайся после первого запроса.
${tgContext}
